...
#define checkTimeoutExpired()                                                  \
    (io_timeout > 0 && ((uint32_t)(millis() - timeout_start_ms) > io_timeout))
...
bool VL53L0X::init(bool io_2v8) {
    ...
    uint8_t __ref_spad_map[7];
    uint8_t * ref_spad_map = &__ref_spad_map[1];
    ...
    __ref_spad_map[0] = GLOBAL_CONFIG_SPAD_ENABLES_REF_0;
    writeMulti(__ref_spad_map, 7);
...
void VL53L0X::writeReg(uint8_t reg, uint8_t value) {
    ...
    buf[0] = reg;
    buf[1] = value;
    last_status = i2c_write_blocking(bus, address, buf, 2, false);
}
...
void VL53L0X::writeReg16Bit(uint8_t reg, uint16_t value) {
    ...
    buf[2] = (uint8_t)(value);
    last_status = i2c_write_blocking(bus, address, buf, 3, false);
}
...
void VL53L0X::writeReg32Bit(uint8_t reg, uint32_t value) {
    uint8_t buf[5];
    buf[0] = reg;
    buf[1] = (uint8_t)(value >> 24); // value highest byte
    buf[2] = (uint8_t)(value >> 16);
    buf[3] = (uint8_t)(value >> 8);
    buf[4] = (uint8_t)(value); // value lowest byte
    last_status = i2c_write_blocking(bus, address, buf, 5, false);
}
...
uint8_t VL53L0X::readReg(uint8_t reg) {
    ...
    uint8_t res;
    int wr = i2c_write_blocking(bus, address, &reg, 1, true);
    last_status = i2c_read_blocking(bus, address, &res, 1, false);
    return res;
}
...
uint16_t VL53L0X::readReg16Bit(uint8_t reg) {
    uint8_t tmp[2];
    int wr = i2c_write_blocking(bus, address, &reg, 1, true);
    last_status = i2c_read_blocking(bus, address, tmp, 2, false);
    uint16_t res = tmp[0] << 8 | tmp[1];
    return res;
}
...
uint32_t VL53L0X::readReg32Bit(uint8_t reg) {
    uint8_t tmp[4];
    int wr = i2c_write_blocking(bus, address, &reg, 1, true);
    last_status = i2c_read_blocking(bus, address, tmp, 4, false);
    uint32_t res =  tmp[0] << 24 | tmp[1] << 16 | tmp[2] << 8 | tmp[3];
    return res;
}

// Write an arbitrary number of bytes from the given array to the sensor,
// starting at the given register
void VL53L0X::writeMulti(uint8_t const *src, uint8_t count) {
    last_status = i2c_write_blocking(bus, address, src, count, false);
}

// Read an arbitrary number of bytes from the sensor, starting at the given
// register, into the given array
void VL53L0X::readMulti(uint8_t reg, uint8_t *dst, uint8_t count) {
    last_status = i2c_write_blocking(bus, address, &reg, 1, true);
    i2c_read_blocking(bus, address, dst, count, false);
}

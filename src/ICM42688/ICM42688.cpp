...
void inline delay(uint32_t ms) { sleep_ms(ms); }
...
int ICM42688::begin() {
    ...
    // // disable inner filters (Notch filter, Anti-alias filter, UI filter
    // block) if (setFilters(false, false) < 0) {
    //   return -7;
    // }
...
int ICM42688::setFilters(bool gyroFilters, bool accFilters) {
    ...
    if (gyroFilters == true) {
        if (writeRegister(UB1_REG_GYRO_CONFIG_STATIC2,
    ...
    if (accFilters == true) {
        ...
        if (writeRegister(UB2_REG_ACCEL_CONFIG_STATIC2, ACCEL_AAF_DISABLE) <
            0) {
            return -6;
        }
    }
...
int ICM42688_FIFO::enableFifo(bool accel, bool gyro, bool temp) {
    // use low speed SPI for register setting
    _useSPIHS = false;
    if (writeRegister(FIFO_EN, (accel * FIFO_ACCEL) | (gyro * FIFO_GYRO) |
                                   (temp * FIFO_TEMP_EN)) < 0) {
        return -2;
    }
    _enFifoAccel = accel;
...
int ICM42688_FIFO::readFifo() {
    ...
    for (size_t i = 0; i < _fifoSize / _fifoFrameSize; i++) {
        ...
        if (_enFifoTemp) {
            // combine into 16 bit values
            int16_t rawMeas = (((int16_t)_buffer[0 + _enFifoAccel * 6]) << 8) |
        ...
        if (_enFifoGyro) {
            ...
            int16_t rawMeas[3];
            rawMeas[0] =
                (((int16_t)_buffer[0 + _enFifoAccel * 6 + _enFifoTemp * 2])
                 << 8) |
                _buffer[1 + _enFifoAccel * 6 + _enFifoTemp * 2];
            rawMeas[1] =
                (((int16_t)_buffer[2 + _enFifoAccel * 6 + _enFifoTemp * 2])
                 << 8) |
                _buffer[3 + _enFifoAccel * 6 + _enFifoTemp * 2];
            rawMeas[2] =
                (((int16_t)_buffer[4 + _enFifoAccel * 6 + _enFifoTemp * 2])
                 << 8) |
